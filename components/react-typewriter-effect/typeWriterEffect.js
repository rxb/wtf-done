...
class TypeWriterEffect extends Component {
  ...
  runAnimation = async (str, erase) => {
    const localNonce = this.globalAnimationNonce = new Object();
    ...
    if (textArr) {
      ...
      // has another animation run started
      if (localNonce !== this.globalAnimationNonce) { return; }

      if(this.props.afterTextComplete){
        this.props.afterTextComplete();
      }
      this.setState({
        blink: true,
      });
